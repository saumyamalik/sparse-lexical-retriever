...
def load_data(opt, tokenizer, val=False):
    datasets = {}
    if val:
        TDIR="/scratch/gpfs/awettig/processed_the_pile/bs6144"
        VALDATASETS=["/scratch/gpfs/awettig/processed_the_pile/bs6144/Wikipedia-val-1.536M/", "/scratch/gpfs/awettig/processed_the_pile/bs6144/ArXiv-val-1.536M/", "/scratch/gpfs/awettig/processed_the_pile/bs6144/Books3-val-1.536M/", "/scratch/gpfs/awettig/processed_the_pile/bs6144/Gutenberg-val-1.536M/", "/scratch/gpfs/awettig/processed_the_pile/bs6144/Github-val-1.536M/", "/scratch/gpfs/awettig/processed_the_pile/bs6144/YoutubeSubtitles-val-1.536M/", "/scratch/gpfs/awettig/processed_the_pile/bs6144/FreeLaw-val-1.536M/", "/scratch/gpfs/awettig/processed_the_pile/bs6144/HackerNews-val-1.536M/"]
        for path in VALDATASETS:
...
#     data = load_from_disk(opt.train_data[0])
#     if data is not None:
#         dataset = Dataset(data, opt.chunk_length, tokenizer, opt)
#     #dataset.set_prob(coeff=opt.sampling_coefficient)
#     return dataset
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        tokens = self.data[index]['input_ids'][start_idx:end_idx]
        ...
        # do it in the model
        # doc_onehot = F.one_hot(k_tokens, self.vocab_size).sum(0)
        # query_onehot = F.one_hot(q_tokens, self.vocab_size).max(0)[0]
